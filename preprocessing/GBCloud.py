...
Description_list = []
Tags_list = []
with open("GBvideos_New.json", 'r') as load_f:
...
writer = pd.ExcelWriter('GBGBGBDATA.xlsx')
...
def handle_excel():
    ...
    wb = load_workbook('GBGBGBDATA.xlsx')  # 打开Excel
    ...
    wb.save('GBresults.xlsx')  # 文件另存为results.xlsx, 也可保存为youtube.xlsx，这样可以覆盖原文件
...
def fetch_description_column():
    wb = load_workbook('GBresults.xlsx')  # 打开Excel
    ws = wb.active  # 得到活动的work sheet
    with open('gbdescription.txt', 'w', encoding='utf-8') as f:
        for cell in ws['L']:
            if cell.value:
                f.write(cell.value)
    wb.close()
...
with open('gbdescription.txt', encoding='utf-8') as f:
...
ws = wb.active
for x in range(10):
    ws['G' + str(x + 2)] = top_10[x][0] + '(' + str(top_10[x][1]) + ')'
print(top_10)
...
wordcloud.to_file('GBdescription.png')
