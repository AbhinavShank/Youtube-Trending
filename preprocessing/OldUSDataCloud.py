...
def word_cloud(file='OLDUSdescription'):
    with open('OLDUSdescription.txt', encoding='utf-8') as f:
    ...
    ws = wb.active
    for x in range(10):
        ws['B' + str(x + 2)] = top_10[x][0] + '(' + str(top_10[x][1]) + ')'
    print(top_10)
    wb.save('词频.xlsx')
...
def cloud_csv():
    for file in [ 'OldUSvideos']:
        print(file + '.csv')
        with open(file + '.csv', encoding='utf-8') as f:
            fw = open('OLDUSdescription.txt', 'w', encoding='utf-8')
