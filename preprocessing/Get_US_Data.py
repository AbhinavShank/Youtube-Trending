...
class YoutubeCrawler():
    first_url = 'https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics&regionCode=US&chart=mostPopular&key=AIzaSyCBJKDBn1bjnfdtQQip5SMK3j5uKeHK7mc&maxResults=50{}'
    ...
    def parse_data(self):
        ...
        for video in self.video_list:
            ...
            video_data['VideoID'] = video.get('id')
            video_data['TrendingDate'] = time.strftime('%Y-%m-%dT%H:%M:%S.000Z',time.localtime(time.time()))
            video_data['Title'] = video.get('snippet').get('title')
            video_data['ChannelTitle'] = video.get('snippet').get('channelTitle')
            video_data['CategoryID'] = video.get('snippet').get('categoryId')
            video_data['PublishTime'] = video.get('snippet').get('publishedAt')
            ...
            video_data['CommentsCount'] = video.get('statistics').get('commentCount')
